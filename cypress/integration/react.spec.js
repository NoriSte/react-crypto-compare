...
describe('CryptoCompare component', () => {
  ...
  const waitAndcheckApikey = (alias = "@cryptocompare", key = apikey) => {
    cy.wait(alias).then(xhr => {
      ...
      expect(xhr.request.headers).to.have.property("Authorization", getApikeyAuthorizationHeader(key))
    })
  }
  ...
  describe('Integration tests', () => {
    ...
    it('Should manage multiple currencies selecting the first one', function() {
      cy.route(url, response).as("cryptocompare");
      cy.mount(<CryptoCompare from={`${from},USD`} to={`${to},BCH`} amount={amount} apikey={apikey} />)
      waitAndcheckApikey();
      checkComponent(amount * response[to], to);
    })
  })
  ...
  describe('E2E tests', () => {
    it('Should convert the currency', function() {
      ...
      cy.mount(<CryptoCompare from={from} to={to} amount={amount} apikey={apikey} />)
      cy.wait("@cryptocompare").its(`response.body.${to}`).then(conversion => checkComponent(amount * conversion, to, `.${defaultClassName}`));
    })
  })
})
